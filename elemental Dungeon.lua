...
local Window = Library.CreateLib("Elemental Dungeon", "Sentinel") -- CREATES THE WINDOW

-- MAIN
local Main = Window:NewTab("Main") -- CREATES THE MAIN TAB
local MobFarmSection = Main:NewSection("Auto Dungeon") -- CREATES THE MOB FARM SECTION

MobFarmSection:NewToggle("Start Auto Dungeon", "Toggles the autofarming of the mobs", function(v) -- CREATES THE START / STOP TOGGLE
    getgenv().autofarm = v
    repeat task.wait() until game:IsLoaded()
    function retry()
...
MobFarmSection:NewToggle(" Auto Heal", "Toggles the auto heal", function(v)
getgenv().heal = v
heal()
end)